package net.translives.app.update;
...
public class DownloadService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        final File apkFile = new File(saveFileName + ".apk");
        setUpNotification();
        new Thread() {
            @Override
            public void run() {
    ...
    private void installApk() {
        File file = new File(saveFileName + ".apk");
        if (!file.exists())
            return;
        Intent intent = new Intent(Intent.ACTION_VIEW);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            intent.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            Uri contentUri = FileProvider.getUriForFile(getApplicationContext(), "net.translives.app.provider", file);
