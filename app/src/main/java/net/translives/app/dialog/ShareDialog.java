package net.translives.app.dialog;
...
import net.translives.app.widget.BottomDialog;
...
import net.translives.app.open.bean.Share;
...
public class ShareDialog extends BottomDialog implements OpenBuilder.Callback,
    ...
    public ShareDialog(@NonNull Activity activity) {
        ...
        mShare = new Share();
        mShare.setAppName(getContext().getResources().getString(R.string.app_name));
    }
    ...
    private List<ShareItem> getAdapterData() {
        ...
        //0.新浪微博
        shareActions.add(new ShareItem(R.mipmap.ic_share_weibo, R.string.platform_sina));

        //1.朋友圈
        shareActions.add(new ShareItem(R.mipmap.ic_share_moments, R.string.platform_wechat_circle));

        //2.微信
        shareActions.add(new ShareItem(R.mipmap.ic_share_wechat, R.string.platform_wechat));

        //3.QQ
        shareActions.add(new ShareItem(R.mipmap.ic_share_qq, R.string.platform_qq));
/*
        if (isOnlyBitmap) {
...
*/
    ...
    public ShareDialog with() {
        mShare.setAppShareIcon(R.mipmap.ic_notification);
        if (mShare.getBitmapResID() == 0)
            mShare.setBitmapResID(R.mipmap.ic_notification);
        return this;
    }
    ...
    public void onItemClick(int position, ShareItem item) {
        ...
        switch (item.iconId) {
            ...
            case R.mipmap.ic_share_weibo:
                ...
                break;
            //朋友圈
            case R.mipmap.ic_share_moments:
    ...
    public static void saveShare(Activity activity, Bitmap bitmap) {
        ...
        try {
            File file = new File(url = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES)
                    .getAbsolutePath() + File.separator + "translives/");
            if (!file.exists())
                file.mkdirs();
    ...
    private void shareSystemImage(String url) {
        ...
        Uri uri = FileProvider.getUriForFile(mActivity, "net.translives.app.provider", new File(url));
